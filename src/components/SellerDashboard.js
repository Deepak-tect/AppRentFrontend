...
function SellerDashboard() {
  ...
  return (
    <div className="seller-dashboard">
      <h2>Your Properties</h2>
      <Link to="/property/new" className="button edit-button">Post a New Property</Link>
      <ul>
        {!properties || properties.length === 0 ? (
          <li>No properties found. <Link to="/property/new">Create a new property</Link></li>
        ) : (
          properties.map((property) => (
            <li key={property._id}>
              <span>{property.title}</span>
              <div>
                <Link to={`/edit-property/${property._id}`} className="button edit-button">Edit</Link>
                <button onClick={() => handleDelete(property._id)} className="button delete-button">Delete</button>
              </div>
            </li>
          ))
        )}
